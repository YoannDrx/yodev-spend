...
export async function getDashboardData(workspaceId: string) {
  if (env.DEMO_DATA_ENABLED === "true") return { ...demoData, excludedCurrencies: [] as Array<{ currency: string; amountMinor: bigint }>, unallocated: [] as Array<{ currency: string; amountMinor: bigint }>, allocationCoverageBps: 10_000, latestSuccessfulSyncAt: null as string | null, fxConversions: [] as Array<{ from: string; to: string; rateScaled: bigint; rateScale: number; rateAt: string; source: string; sourceUrl: string }> };
  ...
  const costRows=await db.select().from(costEntries).where(and(eq(costEntries.workspaceId,workspaceId),gt(costEntries.periodEnd,monthStart),lt(costEntries.periodStart,monthEnd),isNull(costEntries.supersededAt)));
  const openAlerts=await db.select({ id:alerts.id,title:alerts.title,description:alerts.description,billingAccountId:alerts.billingAccountId,type:alerts.type }).from(alerts).where(and(eq(alerts.workspaceId,workspaceId),eq(alerts.status,"open"))).orderBy(desc(alerts.createdAt)).limit(6);
  ...
  const wasteAlerts = await db.select({billingAccountId:alerts.billingAccountId}).from(alerts).where(and(eq(alerts.workspaceId,workspaceId),eq(alerts.status,"open"),inArray(alerts.type,["PAID_PROVIDER_WITHOUT_PROJECT"])));
  const wasteAccounts = new Set(wasteAlerts.map((item)=>item.billingAccountId));
  const wasteSubscriptions=subscriptionRows.filter((row)=>wasteAccounts.has(row.billingAccountId));
  const wasteReporting=convertCostsForReporting([...new Set(wasteSubscriptions.map((row)=>row.currency))].map((currency)=>({id:`waste:${currency}`,amountMinor:sumMonthlyCommitments(wasteSubscriptions,currency).amountMinor,currency,periodStart:now})),rateRows,"EUR");
  const potentialWaste=wasteReporting.converted.reduce((sum,row)=>sum+row.amountMinor,0n);
  ...
  const ledgerAllocations=allocateLedgerCosts(reporting.converted,resourceMappings,billingMappings);const allocationSummary=allocationTotals(ledgerAllocations);const absoluteTotal=reporting.converted.reduce((sum,cost)=>sum+(cost.amountMinor<0n?-cost.amountMinor:cost.amountMinor),0n);const absoluteUnallocated=ledgerAllocations.filter((allocation)=>allocation.projectId===null).reduce((sum,allocation)=>sum+(allocation.amountMinor<0n?-allocation.amountMinor:allocation.amountMinor),0n);const allocationCoverageBps=absoluteTotal===0n?null:Number((absoluteTotal-absoluteUnallocated)*10_000n/absoluteTotal);const latestSuccessfulSyncAt=syncRows.map((row)=>row.lastSuccessfulSyncAt).filter((date):date is Date=>Boolean(date)).sort((left,right)=>right.getTime()-left.getTime())[0]?.toISOString()??null;const allAppliedRates=new Map([...reporting.appliedRates,...commitmentReporting.appliedRates].map((rate)=>[`${rate.quoteCurrency}:${rate.rateAt.toISOString()}:${rate.source}`,rate]));
  return { metrics:{ monthToDate,monthlyCommitment,forecast:monthToDate>forecastCandidate?monthToDate:forecastCandidate,potentialWaste,annualized:monthlyCommitment*12n }, categories:categoryRows.map((row)=>({name:row.name,amount:row.amount,share:positiveCategoryTotal>0n?Number(row.amount*100n/positiveCategoryTotal):0})), excludedCurrencies, allocationCoverageBps,latestSuccessfulSyncAt,fxConversions:[...allAppliedRates.values()].map((rate)=>({from:rate.quoteCurrency,to:rate.baseCurrency,rateScaled:rate.rateScaled,rateScale:rate.rateScale,rateAt:rate.rateAt.toISOString(),source:rate.source,sourceUrl:rate.sourceUrl})), unallocated:[...allocationSummary.unallocated].map(([currency,amountMinor])=>({currency,amountMinor})), discoveries:recentIntegrations.map((item)=>({...item,age:"recent",evidence:evidenceRows.filter((row)=>row.integrationId===item.id).map((row)=>`${row.filePath??"manual"} → ${row.key}`)})), alerts:openAlerts.map((item)=>({id:item.id,title:item.title,detail:item.description,amount:0n})), projects:projectRows.map((item)=>({...item,monthly:allocationSummary.projects.get(item.id)?.get("EUR")??0n,scan:item.lastScan ? new Date(item.lastScan).toISOString() : "—"})) };
  });
}

export async function getPortfolioData(workspaceId: string) {
  if (env.DEMO_DATA_ENABLED === "true") return { ...demoData, clients:demoData.clients.map((client)=>({...client,description:client.description as string | null,status:"active"})),projects:demoData.projects.map((project)=>({...project,status:"active"})), unallocated: [] as Array<{ currency: string; amountMinor: bigint }>, billing: demoData.billing.map((account) => ({ ...account, recurringAmounts:[{currency:"EUR",amountMinor:account.recurring}], allocations: [] as Array<{ projectId: string; allocationBps: number }> })) };
  return withAuthorizedWorkspace(workspaceId, async (db) => {
  const now=new Date();const monthStart=new Date(Date.UTC(now.getUTCFullYear(),now.getUTCMonth(),1));const monthEnd=new Date(Date.UTC(now.getUTCFullYear(),now.getUTCMonth()+1,1));
  const clientRows=await db.select({id:clients.id,name:clients.name,description:clients.description,status:clients.status,projects:sql<number>`count(${projects.id})::int`}).from(clients).leftJoin(projects,eq(projects.clientId,clients.id)).where(eq(clients.workspaceId,workspaceId)).groupBy(clients.id).orderBy(clients.name);
  ...
  const serviceRows=await db.select({id:providers.id,slug:providers.slug,name:providers.name,category:providers.category,projects:sql<number>`count(distinct ${projectIntegrations.projectId})::int`,status:sql<string>`min(${projectIntegrations.lifecycleStatus})`,lastDetected:sql<string | null>`max(${projectIntegrations.lastDetectedAt})`}).from(projectIntegrations).innerJoin(providers,eq(providers.id,projectIntegrations.providerId)).where(eq(projectIntegrations.workspaceId,workspaceId)).groupBy(providers.id).orderBy(providers.name);
  const accountRows=await db.select({id:billingAccounts.id,name:billingAccounts.name,provider:providers.name,providerId:providers.id,owner:billingAccounts.ownerType,status:billingAccounts.status}).from(billingAccounts).innerJoin(providers,eq(providers.id,billingAccounts.providerId)).where(eq(billingAccounts.workspaceId,workspaceId)).orderBy(billingAccounts.name);
  const subscriptionRows=await db.select().from(subscriptions).where(and(eq(subscriptions.workspaceId,workspaceId),eq(subscriptions.status,"active")));
  const currentCosts=await db.select({id:costEntries.id,amountMinor:costEntries.amountMinor,currency:costEntries.currency,projectId:costEntries.projectId,externalResourceId:costEntries.externalResourceId,billingAccountId:costEntries.billingAccountId,periodStart:costEntries.periodStart,periodEnd:costEntries.periodEnd}).from(costEntries).where(and(eq(costEntries.workspaceId,workspaceId),gt(costEntries.periodEnd,monthStart),lt(costEntries.periodStart,monthEnd),isNull(costEntries.supersededAt)));
  const resourceMappings=await db.select({externalResourceId:externalResourceProjects.externalResourceId,projectId:externalResourceProjects.projectId,allocationBps:externalResourceProjects.allocationBps,allocationMethod:externalResourceProjects.allocationMethod,effectiveFrom:externalResourceProjects.effectiveFrom,effectiveTo:externalResourceProjects.effectiveTo}).from(externalResourceProjects).where(eq(externalResourceProjects.workspaceId,workspaceId));
  const billingMappings=await db.select({billingAccountId:billingAccountProjects.billingAccountId,projectId:billingAccountProjects.projectId,allocationBps:billingAccountProjects.allocationBps,allocationMethod:billingAccountProjects.allocationMethod,effectiveFrom:billingAccountProjects.effectiveFrom,effectiveTo:billingAccountProjects.effectiveTo}).from(billingAccountProjects).where(eq(billingAccountProjects.workspaceId,workspaceId));
  ...
  return { clients:clientRows,projects:projectRows.map((row)=>({...row,monthly:allocationSummary.projects.get(row.id)?.get("EUR")??0n,scan:row.lastScan ? new Date(row.lastScan).toISOString() : "—"})),services:serviceRows.map((row)=>({...row,monthly:providerCosts.get(row.id)??0n,last:row.lastDetected ? new Date(row.lastDetected).toISOString() : "—"})),unallocated:[...allocationSummary.unallocated].map(([currency,amountMinor])=>({currency,amountMinor})),billing:accountRows.filter((row)=>row.status==="active").map((row)=>{const accountSubscriptions=subscriptionRows.filter((item)=>item.billingAccountId===row.id);return {...row,allocations:billingMappings.filter((mapping)=>mapping.billingAccountId===row.id&&mapping.effectiveTo===null).map(({projectId,allocationBps})=>({projectId,allocationBps})),recurring:sumMonthlyCommitments(accountSubscriptions,"EUR").amountMinor,recurringAmounts:[...new Set(accountSubscriptions.map((item)=>item.currency))].map((currency)=>({currency,amountMinor:sumMonthlyCommitments(accountSubscriptions,currency).amountMinor})),renewal:accountSubscriptions.map((item)=>item.renewalDate).filter((date):date is Date=>Boolean(date)).sort((a,b)=>a.getTime()-b.getTime())[0]?.toISOString().slice(0,10)??"—"};}) };
  });
}
...
export async function getClientDetail(workspaceId: string, clientId: string) {
  if (env.DEMO_DATA_ENABLED === "true") {
    const client = demoData.clients.find((item) => item.id === clientId) ;
  ...
  return withAuthorizedWorkspace(workspaceId, async (db) => {
    ...
    return { client, projects: projectRows.map((row) => ({ ...row, monthly: 0n, scan: row.lastScan ? new Date(row.lastScan).toISOString() : null })) };
  });
}
