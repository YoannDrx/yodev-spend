...
import { assertManualCostPeriodOpen, lockBillingAccount } from "@/server/billing/reconciliation";

export async function createBillingAccount(formData: FormData) {
  const input=z.object({locale:z.enum(["fr","en"]),name:z.string().trim().min(2).max(180),providerSlug:z.string().min(1),ownerType:z.enum(["workspace","client","shared"]),clientId:z.union([z.literal(""),z.uuid()]).optional(),projectIds:z.array(z.uuid()).max(50),allocationMethod:z.enum(["equal","manual"]).default("equal"),amountMinor:z.coerce.bigint().nonnegative(),currency:z.string().trim().toUpperCase().refine((value)=>Intl.supportedValuesOf("currency").includes(value)),billingInterval:z.enum(["month","year"])}).superRefine((value,context)=>{if(value.ownerType==="client"&&!value.clientId)context.addIssue({code:"custom",path:["clientId"],message:"A client owner is required."});if(value.ownerType==="shared"&&!value.projectIds.length)context.addIssue({code:"custom",path:["projectIds"],message:"Select at least one project for a shared account."});}).parse({...Object.fromEntries(formData),projectIds:formData.getAll("projectIds"),currency:String(formData.get("currency")??"EUR").toUpperCase(),amountMinor:formData.has("amount")?parseMoneyInput(String(formData.get("amount")),String(formData.get("currency")??"EUR")):formData.get("amountMinor")});
  ...
  await withWorkspaceMutation(context.workspaceId,async(tx)=>{
    ...
    const [account]=await tx.insert(billingAccounts).values({workspaceId:context.workspaceId,providerId:provider.id,clientId:input.ownerType==="client"?input.clientId||null:null,name:input.name,ownerType:input.ownerType,defaultCurrency:input.currency}).returning({id:billingAccounts.id});
    ...
    await tx.insert(subscriptions).values({workspaceId:context.workspaceId,billingAccountId:account.id,name:input.name,billingModel:input.billingInterval==="year"?"fixed_yearly":"fixed_monthly",amountMinor:input.amountMinor,currency:input.currency,billingInterval:input.billingInterval,status:"active"});
...
export async function updateBillingAccountAllocation(formData: FormData) {
  ...
  await withWorkspaceMutation(context.workspaceId, async (db) => {
    await lockBillingAccount(db, context.workspaceId, input.billingAccountId);
...
export async function createCostEntry(formData: FormData) {
  const input = z.object({
    ...
    currency: z.string().trim().toUpperCase().refine((value)=>Intl.supportedValuesOf("currency").includes(value)),
    ...
    path: ["periodEnd"],
  }).parse({...Object.fromEntries(formData),amountMinor:formData.has("amount")?parseMoneyInput(String(formData.get("amount")),String(formData.get("currency")??"EUR")):formData.get("amountMinor")});

  const context = await requireWorkspaceMutationContext(input.locale);
  await withWorkspaceMutation(context.workspaceId, async (db) => {
  await assertManualCostPeriodOpen(db, context.workspaceId, input);
  const [account] = await db.select({ id: billingAccounts.id })
    .from(billingAccounts)
    .where(and(eq(billingAccounts.id, input.billingAccountId), eq(billingAccounts.workspaceId, context.workspaceId), eq(billingAccounts.status, "active")))
    .limit(1);
  if (!account) throw new Error("Billing account not found.");
