...
export default async function DashboardPage({ params }: PageProps<"/[locale]/dashboard">) {
  ...
  return <>
    ...
    <div className="dashboard-grid">
      ...
      <section id="alerts" className="panel"><div className="panel-head"><h2>{t("attention")}</h2><Link href="/discoveries">{t("view")} <ArrowRight size={11} /></Link></div><div className="panel-body">
        {data.alerts.map((item) => <div className="list-row" key={item.id}><div className="row-main"><span className="provider-dot"><AlertTriangle size={14} /></span><div><strong>{item.title}</strong><small>{item.detail}</small></div></div><div className="row-value">{item.amount ? formatMoney(item.amount, "EUR", locale) : "—"}{!item.id.startsWith("demo-") ? <div className="topbar-actions"><ActionForm action={reviewAlertAction}><input type="hidden" name="locale" value={locale} /><input type="hidden" name="alertId" value={item.id} /><input type="hidden" name="decision" value="resolve" /><button className="button button-small" type="submit">{t("resolve")}</button></ActionForm><ActionForm action={reviewAlertAction}><input type="hidden" name="locale" value={locale} /><input type="hidden" name="alertId" value={item.id} /><input type="hidden" name="decision" value="dismiss" /><button className="button button-small" type="submit">{t("dismiss")}</button></ActionForm></div> : null}</div></div>)}
