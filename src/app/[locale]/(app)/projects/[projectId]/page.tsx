...
export default async function ProjectDetailPage({ params }: PageProps<"/[locale]/projects/[projectId]">) {
  ...
  if (!data) notFound();
  const { project } = data;
  const action = project.status !== "archived" && !project.id.startsWith("demo-") ? <ActionForm action={archiveProject}><input type="hidden" name="locale" value={locale}/><input type="hidden" name="projectId" value={projectId}/><button className="button" type="submit"><Archive size={13}/>{t("archive")}</button></ActionForm> : <StatusBadge status="archived"/>;
  return <>
    <PageHeader title={project.name} subtitle={`${project.client} · ${project.repositories} ${t("repositories").toLowerCase()}`} action={action}/>
    {!project.id.startsWith("demo-") ? <details className="panel" style={{marginBottom:16}}><summary className="panel-head"><h2>{t("edit")}</h2></summary><ActionForm action={updateProject} className="form-card"><input type="hidden" name="locale" value={locale}/><input type="hidden" name="projectId" value={projectId}/><label className="field"><span>{t("name")}</span><input name="name" defaultValue={project.name} required/></label><label className="field"><span>{t("description")}</span><textarea name="description" defaultValue={project.description??""}/></label><label className="field"><span>{t("status")}</span><select name="status" defaultValue={project.status}><option value="active">{t("active")}</option><option value="maintenance">{t("maintenance")}</option><option value="archived">{t("archived")}</option></select></label><button className="button button-primary" type="submit">{t("save")}</button></ActionForm></details> : null}
    <div className="dashboard-grid">
      <section className="panel"><div className="panel-head"><h2>{t("repositories")}</h2></div><div className="panel-body">{data.repositories.map((repository)=><div className="list-row" key={repository.id}><div className="row-main"><span className="provider-dot"><GitBranch size={15}/></span><div><strong>{repository.fullName}</strong><small>{repository.defaultBranch} · {repository.lastScan ? t("lastScanValue",{date:repository.lastScan}) : t("neverScanned")}</small></div></div><div className="topbar-actions"><StatusBadge status="active" label={repository.scanEnabled?t("enabled"):t("disabled")}/>{repository.id==="demo-repository"?<span className="status-badge">Demo</span>:<ActionForm action={scanRepositoryAction}><input type="hidden" name="locale" value={locale}/><input type="hidden" name="repositoryId" value={repository.id}/><input type="hidden" name="mode" value="quick"/><input type="hidden" name="force" value="true"/><button className="button button-small" type="submit">{t("scanNow")}</button></ActionForm>}</div></div>)}</div></section>
