...
export function InvoiceForm({ locale, accounts, labels }: {
  ...
  return <details className="panel" style={{ marginBottom: 16 }}>
    ...
    <ActionForm action={createManualInvoiceAction} className="form-card" style={{ border: 0, margin: 0 }}>
      ...
      <div className="field"><label htmlFor="invoice-start">{labels.periodStart}</label><input id="invoice-start" name="periodStart" type="date" defaultValue={monthStart} required /></div>
      <div className="field"><label htmlFor="invoice-end">{labels.periodEnd}</label><input id="invoice-end" name="periodEnd" type="date" defaultValue={monthEnd} required /></div>
      <div className="field"><label htmlFor="invoice-total">{labels.amountMinor}</label><input id="invoice-total" name="amount" type="text" inputMode="decimal" placeholder="0,00" required /></div>
